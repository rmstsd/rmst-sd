...
const require$$2$1 = require("node:util");
const require$$1$4 = require("node:events");
...
function requireSetprototypeof$1() {
  ...
  setprototypeof$1 = Object.setPrototypeOf || ({ __proto__: [] } instanceof Array ? setProtoOf : mixinProperties);
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (Array.isArray(value)) {
          ...
          for (let i = 0; i < lgth; i++) {
            addKey(key, value[i], searchParams2);
          }
        } else {
          addKey(key, value, searchParams2);
        }
      }
      return searchParams2.toString();
    },
    parse: (str) => {
      const searchParams2 = new URLSearchParams(str);
      const obj = {};
      for (const key of searchParams2.keys()) {
        const values = searchParams2.getAll(key);
...
function requireOnly() {
  if (hasRequiredOnly) return only;
  hasRequiredOnly = 1;
  only = (obj, keys2) => {
    const ret = {};
...
function requireRequest() {
  ...
  (function(module2) {
    ...
    module2.exports = {
      ...
      get charset() {
        try {
          const { parameters } = contentType2.parse(this.req);
       ...
       */
      get accept() {
        return this._accept || (this._accept = accepts2(this.req));
      },
      /**
       * Set accept object.
       *
...
function requireResponse() {
  ...
  (function(module2) {
    ...
    const getType = requireMimeTypes().contentType;
    ...
    module2.exports = {
      ...
      set status(code) {
        ...
        if (this.req.httpVersionMajor < 2) this.res.statusMessage = statuses2.message[code];
        if (this.body && statuses2.empty[code]) this.body = null;
      },
      /**
       ...
       */
      get message() {
        return this.res.statusMessage || statuses2.message[this.status];
      },
      /**
      ...
      set body(val) {
        ...
        const cleanupPreviousStream = () => {
          if (original && isStream2(original)) {
            original.once("error", () => {
            });
            if (!isStream2(val)) {
              destroy(original);
        ...
        if (val == null) {
          if (!statuses2.empty[this.status]) {
      ...
      get length() {
        ...
        const { body } = this;
        if (!body || isStream2(body)) return void 0;
      ...
      redirect(url) {
        ...
        if (!statuses2.redirect[this.status]) this.status = 302;
      ...
      attachment(filename, options) {
        ...
        this.set("Content-Disposition", contentDisposition2(filename, options));
      },
      /**
...
function requireKeys() {
  if (hasRequiredKeys) return keys.exports;
  hasRequiredKeys = 1;
  (function(module2, exports$1) {
...
var hasRequiredIs_arguments;
function requireIs_arguments() {
  if (hasRequiredIs_arguments) return is_arguments.exports;
  hasRequiredIs_arguments = 1;
  (function(module2, exports$1) {
...
function requireDeepEqual() {
  ...
  var objectKeys = requireKeys();
  var isArguments = requireIs_arguments();
  var deepEqual$1 = deepEqual.exports = function(actual, expected, opts) {
    if (!opts) opts = {};
    if (actual === expected) {
      return true;
...
function requireKeygrip() {
  ...
  function Keygrip(keys2, algorithm, encoding2) {
    ...
    if (!(this instanceof Keygrip)) return new Keygrip(keys2, algorithm, encoding2);
    if (!keys2 || !(0 in keys2)) {
      throw new Error("Keys must be provided.");
    }
    function sign(data, key) {
      return crypto.createHmac(algorithm, key).update(data).digest(encoding2).replace(/\/|\+|=/g, function(x) {
        return { "/": "_", "+": "-", "=": "" }[x];
      });
    }
    this.sign = function(data) {
      return sign(data, keys2[0]);
    };
    ...
    this.index = function(data, digest) {
      for (var i = 0, l = keys2.length; i < l; i++) {
        if (compare(digest, sign(data, keys2[i]))) {
          return i;
        }
      }
      return -1;
...
function requireCookies() {
  ...
  function Cookies(request2, response2, options) {
    if (!(this instanceof Cookies)) return new Cookies(request2, response2, options);
    this.secure = void 0;
    this.request = request2;
...
function requireContext() {
  if (hasRequiredContext) return context.exports;
  ...
  (function(module2) {
    ...
    const proto = module2.exports = {
      ...
      onerror(err) {
        ...
        if (typeof statusCode !== "number" || !statuses2.message[statusCode]) statusCode = 500;
        const code = statuses2.message[statusCode];
...
function requireApplication() {
  ...
  const Emitter = require$$1$4;
  const Stream = require$$2$3;
  ...
  application.exports = class Application extends Emitter {
     ...
     */
    toJSON() {
      return only2(this, ["subdomainOffset", "proxy", "env"]);
    }
    /**
     * Inspect implementation.
     *
...
function requireResolvePath() {
  ...
  function resolvePath(rootPath, relativePath) {
    ...
    if (pathIsAbsolute2.posix(path) || pathIsAbsolute2.win32(path)) {
      throw createError(400, "Malicious Path");
    }
...
function requireKoaSend() {
  if (hasRequiredKoaSend) return koaSend;
