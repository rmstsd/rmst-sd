// 根据弧度 求椭圆上的点的坐标
export function findPointInEllipseByRadian(ellipse, theta) {
  ...
  const 分子 = theta > Math.PI / 2 && theta < (Math.PI / 2) * 3 ? -gs_b - sqrtValue : -gs_b + sqrtValue
  const 分母 = 2 * gs_a
  const ans = 分子 / 分母
...
export function standardRadian(radian) {
...
// 点在 在椭圆内,上,外
export function isInPosition(p, cx, cy, radiusX, radiusY) {
...
// 依据椭圆的参数方程
function getPointByTheta(ellipse, theta) {
...
// 计算焦点
export function focalPoint(ellipse) {
  // 焦点在 x 轴
